...
class CrossStrategy:
    ...
    def __post_init__(self) -> None:
        assert self.max_half_sweeps > 0
        assert self.error_evals > 0
...
class CrossInterpolation:
    ...
    def sample_fiber(self, k: int) -> Tensor3:
        """Samples the fiber of the MPS at site k."""
    ...
    def sample_superblock(self, k: int) -> Tensor4:
        """Samples the superblock of the MPS at sites k and k+1."""
        i_l, i_g = self.I_l[k], self.I_g[k + 1]
        i_s1, i_s2 = self.I_s[k], self.I_s[k + 1]
        mps_indices = self.combine_indices(i_l, i_s1, i_s2, i_g)
        return self.black_box[mps_indices].reshape((len(i_l), len(i_s1), len(i_s2), len(i_g)))
...
class CrossError:
    ...
    def __init__(self, cross_strategy: CrossStrategy):
        ...
        self.error_evals = cross_strategy.error_evals
