...
DEFAULT_CROSS_DMRG_STRATEGY = SIMPLIFICATION_STRATEGY.replace(normalize=False, tolerance=DEFAULT_TOLERANCE**2, simplification_tolerance=DEFAULT_TOLERANCE**2)
...
def _update_cross(
    ...
    if left_to_right:
        if k < cross.sites - 2:
            ...
            I, G = maxvol_square(
                Q, cross_strategy.maxiter_maxvol, cross_strategy.tol_maxvol_square  # type: ignore
            )
        ...
        if k > 0:
            ...
            Q, _ = scipy.linalg.qr(R.T, mode="economic", overwrite_a=True, check_finite=False)  # type: ignore
            I, G = maxvol_square(
                Q, cross_strategy.maxiter_maxvol, cross_strategy.tol_maxvol_square  # type: ignore
            )
