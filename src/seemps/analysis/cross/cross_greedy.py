...
class CrossInterpolationGreedy(CrossInterpolation):
    ...
    def update_indices(self, k: int, j_l: np.intp | IndexVector, j_g: np.intp | IndexVector) -> None:
...
def _update_cross(
    ...
    fn = lambda A, B: np.abs(A - B)  # TODO: Consider other functions
    diff = fn(A_random, B_random)
    ...
    for i in range(cross_strategy.max_iters):
        ...
        new_j_g = np.argmax(fn(r_A, r_B))
        if new_j_g == j_g:
            break
        j_g = new_j_g

    # Add pivot if its error is large enough
    pivot_error = fn(c_A[j_l], c_B[j_l])
